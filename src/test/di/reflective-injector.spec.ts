...
describe(`injector`, () => {
  ...
  it('should instantiate an object after a failed attempt', () => {
    let injector = createInjector([Car, { provide: Engine, useClass: BrokenEngine }]);

    expect(() => injector.get(Car)).toThrowError(
      'Broken Engine: Error during instantiation of Engine! (Car -> Engine).'
    );
    injector = createInjector([Car, { provide: Engine, useClass: Engine }]);
...
describe('displayName', () => {
  it('should work', () => {
    expect(
      (<ReflectiveInjector_>ReflectiveInjector.resolveAndCreate([Engine, BrokenEngine])).displayName
